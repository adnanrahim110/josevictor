    ...
    <section className="py-24 lg:py-32 bg-primary-50 border-t border-primary-100">
      <Container>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {/* Featured LEGO Serious Play Card */}
          <motion.div
            ...
            <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              ...
              <div className="flex justify-center lg:justify-end">
                <div className="w-48 h-48 sm:w-64 sm:h-64 rounded-[48px] bg-primary-900/50 flex items-center justify-center border border-primary-800 relative shadow-inner">
                  ...
                  <Blocks className="w-24 h-24 sm:w-32 sm:h-32 text-secondary-400 drop-shadow-2xl transform transition-transform duration-700 group-hover:scale-110 group-hover:rotate-6" strokeWidth={1.5} />
                </div>
              </div>
            </div>
          </motion.div>
...
function CatalogCard({ title, desc, icon, delay }: { title: string, desc: string, icon: React.ReactNode, delay: number }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 30 }}
      whileInView={{ opacity: 1, y: 0 }}
      ...
      className="bg-white p-8 sm:p-10 rounded-[32px] border border-primary-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] transition-all duration-500 hover:-translate-y-1 flex flex-col h-full group"
    >
      ...
      <p className="text-primary-600 leading-relaxed flex-1">
        {desc}
      </p>
    </motion.div>
  );
